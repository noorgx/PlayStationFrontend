...
const CustomNavbar = ({ user, onLogout }) => {
  ...
  return (
    ...
    <Navbar bg="dark" variant="dark" expand="lg" className="mb-4"> {/* Add margin-bottom to the Navbar */}
      <Container>
        <Navbar.Brand as={Link} to="/" className="me-4">Playstation</Navbar.Brand> {/* Add margin-right to the brand */}
        <Navbar.Toggle aria-controls="basic-navbar-nav" />
        <Navbar.Collapse id="basic-navbar-nav">
          {user ? (
            <>
              <Nav className="me-auto">
                <Nav.Link as={Link} to="/rooms" className="me-3 d-flex align-items-center">
                  <FaDoorOpen className="me-2" /> الغرف
                </Nav.Link> {/* Add margin-right to Nav.Link */}
                <Nav.Link as={Link} to="/machines" className="me-3 d-flex align-items-center">
                  <FaGamepad className="me-2" /> الأجهزة
                </Nav.Link> {/* Add margin-right to Nav.Link */}
                <Nav.Link as={Link} to="/food-drinks" className="me-3 d-flex align-items-center">
                  <FaUtensils className="me-2" /> المأكولات و المشروبات
                </Nav.Link> {/* Add margin-right to Nav.Link */}
                <Nav.Link as={Link} to="/calc" className="me-3 d-flex align-items-center">
                  <FaCalculator className="me-2" /> الحاسبة
                </Nav.Link> {/* Add margin-right to Nav.Link */}
                {user.role === 'admin' && (
                  <>
                    <Nav.Link as={Link} to="/quotes" className="me-3 d-flex align-items-center">
                      <FaQuoteRight className="me-2" /> الفواتير
                    </Nav.Link> {/* Add margin-right to Nav.Link */}
                    <Nav.Link as={Link} to="/storage" className="me-3 d-flex align-items-center">
                      <FaWarehouse className="me-2" /> المخزن
                    </Nav.Link> {/* Add margin-right to Nav.Link */}
                    <Nav.Link as={Link} to="/admin" className="me-3 d-flex align-items-center">
            ...
            <Nav className="ms-auto">
              <Nav.Link as={Link} to="/register" className="me-3 d-flex align-items-center">
